<?php

namespace App\Services\Verify\Storehouse;

use App\Http\Traits\FilesTrait;
use App\Jobs\auth\storehouse\AcceptRequestJob;
use App\Jobs\auth\storehouse\RejectRequestJob;
use App\Models\UserFile;
use App\Models\VerifyRequest;
use App\Services\BaseService;
use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\DB;

class VerifyRequestsService extends BaseService
{
    use FilesTrait;

    /**
     *
     * @return Response
     */
    public function getVerifyRequest(): Response
    {
        $response = array();

        $verifyRequests = VerifyRequest::all();

        foreach ($verifyRequests as $verifyRequest) {
            $user = $verifyRequest->user;

            $storehouse = $user->storehouse;

            if ($storehouse)
                $response[] = [
                    "user_id" => $verifyRequest->user_id,
                    "store_id" => $storehouse->id,
                    "store_name" => $storehouse->name,
                    'latitude' => $storehouse->latitude,
                    'longitude' => $storehouse->longitude,
                    "created_at" => $storehouse->created_at,
                    "updated_at" => $storehouse->updated_at,
                    "firstname" => $user->firstname,
                    "lastname" => $user->lastname,
                    "email" => $user->email,
                    "phone_number" => $user->phone_number,
                ];

        }

        return $this->customResponse(true, 'get verify requests storehouse success', $response);
    }

    /**
     * @param Request
     * @return Response
     */
    public function getFileRequest($request): Response
    {
        $verifyrequest = $request->get('verifyrequest');

        $user = $verifyrequest->user;

        $requestFiles = $user->userfiles;

        $response = [
            'requestFiles' => $requestFiles,
        ];

        return $this->customResponse(true, 'get file request success', $response);
    }

    /**
     * @param Request
     * @return Response
     */
    public function acceptRequest($request): Response
    {
        DB::beginTransaction();

        $verifyrequest = $request->get('verifyrequest');

        $user = $verifyrequest->user;

        if($user->user_type == 3) {
            VerifyRequest::where('user_id', $verifyrequest->user_id)->delete();

            $storehouse = $user->storehouse;

            $response = [
                'latitude' => $storehouse->latitude,
                'longitude' => $storehouse->longitude,
                'user_email' => $user->email,
            ];

            try {

                AcceptRequestJob::dispatch($response)->onQueue('main');
                AcceptRequestJob::dispatch($response)->onQueue('store');

            } catch (\Exception $e) {
                DB::rollBack();
                return $this->customResponse(false, 'Bad Internet', null, 504);
            }
            DB::commit();


            return $this->customResponse(true, 'تم الموافقة على الطلب بنجاح');
        }

        return $this->customResponse(true, 'هذا الطلب غير مخصص لمستودع');
    }

    /**
     * @param Request
     * @return Response
     */
    public function rejectRequest($request): Response
    {
        DB::beginTransaction();

        $verifyrequest = $request->get('verifyrequest');

        $user = $verifyrequest->user;

        VerifyRequest::where('user_id', $verifyrequest->user_id) -> delete();

        if($user->user_type == 3) {
            $userfiles = UserFile::where('user_id', $verifyrequest->user_id)->get();

            foreach ($userfiles as $userfile) {
                $this->destoryFile($userfile->path);
                $userfile->delete();
            }
            $response = [
                'user_email' => $user->email,
            ];

            try {

                RejectRequestJob::dispatch($response)->onQueue('main');
                RejectRequestJob::dispatch($response)->onQueue('store');

            } catch (\Exception $e) {
                DB::rollBack();
                return $this->customResponse(false, 'Bad Internet', null, 504);
            }

            DB::commit();

            return $this->customResponse(true, 'تم رفض هذا الطلب بنجاح');
        }

        return $this->customResponse(true, 'هذا الطلب غير مخصص لمستودع');
    }
}
